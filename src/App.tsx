  ...
  return <div className="app-shell">
    ...
    <main className="editor-main">
      ...
      <section className="center-panel panel">
        ...
        <WorkspacePanel key={`${server.target}:${baseline?.binding || ''}:${runName}:${branchId}:${planId}:prompts`} active={page === 'edit' && editTab === 'prompt'}><PromptWorkspace plan={plan} selected={selected} active={page === 'edit' && editTab === 'prompt' && !workflowOpen} context={authoringContext} available={baseline?.capabilities?.promptToolsVersion === 1} editable={canAuthorPlan} duration={Number(activeShot?.duration_seconds ?? plan?.defaults?.duration_seconds ?? planNode?.inputs.default_duration_seconds) || (activeShot ? rawFrames(activeShot, plan!, planNode?.inputs || {}) / 24 : 6)} select={setSelected} inspect={inspectPrompt} stage={value => editPlan({ type: 'scene-prompt', index: selected, value })}/></WorkspacePanel>
