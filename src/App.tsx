  ...
  return <div className="app-shell">
    ...
    <main className="editor-main">
      ...
      <section className="center-panel panel">
        ...
        {page === 'generate' && <GenerationPanel key={`${server.target}:${baseline?.binding || runName}`} pausedReason={wrongTab ? 'Return to the attached ComfyUI workflow to watch its preview.' : live.requested && live.status !== 'attached' ? 'Keep the attached ComfyUI tab open to watch previews.' : ''} installed={Boolean(server.schemas.PreviewRelay)} connected={server.connected} online={server.socketOnline} channels={previewChannels} subscribe={server.subscribePreviewRelay} target={server.target} reconnect={() => void server.connect(server.target)}/>}
      ...
      <div className="inspector-pane" hidden={!workspace.fitted.inspector}><Inspector workflow={workflow} sourceBridgeReady={sourceBridgeReady} schemas={schemas} plan={plan} planId={planId} selected={selected} nodeId={selectedNode || planId} tab={inspectorTab} setTab={setInspectorTab} updateInput={updateInput} updatePlan={updatePlan} select={select} selectNode={selectNode} editJson={() => setModal('plan-json')} report={report} connected={server.connected}/></div>
    </main>
