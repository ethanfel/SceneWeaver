...
import { savedFrameSource, checkpointMedia, checkpointThumbnailUrl, playbackSegments, parseSubtitles } from './lib/playback';
...
export default function App() {
  ...
  const [captureFrame, setCaptureFrame] = useState<{ frame: CapturedFrame; scope: string } | null>(null);
  const captureScope = JSON.stringify([server.target, baseline?.binding, runName, branchId, planId]);
  useEffect(() => setCaptureFrame(null), [captureScope]);
  ...
  return <div className="app-shell">
    ...
    {captureFrame?.scope === captureScope && projectBinding.status === 'bound' && projectBinding.managerId && <FrameCapture key={captureScope} frame={captureFrame.frame} project={runName} branchId={branchId} manager={projectBinding.managerId} planId={planId} editable={canWriteLive && !draftEdits.length} command={live.command} close={() => setCaptureFrame(null)}/>}
