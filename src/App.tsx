...
import { checkpointMedia, playbackSegments, parseSubtitles } from './lib/playback';
...
export default function App() {
  ...
  const select = (index: number) => { setPage('edit'); setSelected(Math.max(0, index)); setPreviewOverride(null); setCurrentTime(0); setTakeChoice('presentation'); };
  ...
  const showPreview = (url: string, name: string, details: Partial<PreviewMedia> = {}) => { if (details.scene) setSelected(details.scene - 1); setPreviewOverride({ url, name, ...details }); setPage('edit'); setCurrentTime(0); setTakeChoice('presentation'); };
  ...
  return <div className="app-shell">
    <header className="app-header"><div className="brand"><div className="brand-mark"><Layers size={22}/></div><strong>SceneWeaver</strong><span className="alpha">COMPANION 0.2.1</span></div>
    ...
    <Timeline plan={plan} inputs={planNode?.inputs || {}} checkpoints={server.checkpoints} editorial={server.editorial} selected={selected} select={select} add={addScene} currentTime={currentTime} onSeek={seconds => { if (video.current) video.current.currentTime = seconds; }}/>
