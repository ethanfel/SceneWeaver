...
export const planNodes = (prompt: Prompt) => Object.entries(prompt).filter(([, node]) => ['MiniMaxH3ChainPlan', 'MiniMaxH3ChainPlanStudio'].includes(node.class_type) && typeof node.inputs.plan_json === 'string');
