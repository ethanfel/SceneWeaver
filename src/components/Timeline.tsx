...
import { playbackSegments } from '../lib/playback';

const colors = ['#53748c', '#697a62', '#8d705d', '#796789', '#5c8583', '#8e6d7c'];
export function Timeline({ plan, inputs, checkpoints, editorial, selected, select, add, currentTime, onSeek }: { plan: Plan | null; inputs: Record<string, Value>; checkpoints: Checkpoint[]; editorial?: Editorial | null; selected: number; select: (i: number) => void; add: () => void; currentTime: number; onSeek: (seconds: number) => void }) {
