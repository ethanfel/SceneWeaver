...
const labels: Record<string, string> = { patch: 'Apply draft', queue: 'Queue workflow', 'workflow-save': 'Save workflow', 'take-final-cut': 'Choose final cut', 'checkpoint-activate': 'Restore checkpoint', 'asset-update': 'Edit asset', 'asset-upload': 'Upload asset', 'asset-import': 'Import asset', 'asset-audio-tracks': 'Apply audio tracks' };
...
export function CommandHistory({ receipts, binding, connected, check, refresh }: { receipts: CommandReceipt[]; binding: string; connected: boolean; check: (id: string) => Promise<unknown>; refresh: () => Promise<unknown> }) {
  ...
  return <details className="command-history"><summary>Actions · {statuses[visible[0].status]}</summary><div className="command-history-list">
    ...
    {visible.slice(0, 12).map(receipt => <article key={receipt.id}><div><strong>{labels[receipt.action] || receipt.action}</strong><span>{statuses[receipt.status]}</span><time>{new Date(receipt.startedAt).toLocaleTimeString()}</time></div>{receipt.project && <small>{receipt.project}{receipt.branch && ` · ${receipt.branch === 'main' ? 'Original' : receipt.branch.slice(0, 8)}`}</small>}{receipt.message && <p>{receipt.message}</p>}{['running', 'uncertain', 'partial'].includes(receipt.status) && <button disabled={!connected || Boolean(checking)} onClick={async () => {
