...
export function LibraryOrganizer(p: { catalog: ProjectCatalog; selection: LibrarySelection; editable: boolean; visibleIds: string[]; mentions: (asset: ProjectAsset) => string[]; mutate: LibraryMutation; close: () => void }) {
  ...
  const origin = asset?.source_origin as { project?: string; asset_id?: string; parent_asset_id?: string; kind?: string; scene?: number; revision?: string; time_seconds?: number } | undefined;
  ...
  return <Modal title={asset ? `Organize ${asset.tag}` : folder ? `Folder: ${folder.name}` : 'New project folder'} onClose={p.close}>
    <div className="modal-body library-organizer">
      ...
      {asset ? <>
        ...
        {origin?.kind === 'saved_frame' ? <p>Captured from {origin.project} · scene {origin.scene} · {origin.time_seconds?.toFixed(3)} s · revision {origin.revision?.slice(0, 8)}</p> : origin && <p>Copied from {origin.project} · asset {origin.asset_id}{origin.parent_asset_id ? ` · source parent ${origin.parent_asset_id}` : ''}</p>}
