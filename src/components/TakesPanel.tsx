...
export function TakesPanel({ project, branchId, server, planId, scene, workflow, connected, editable: canEdit, capabilities, savedVersion, command, preview, changed, report }: Props) {
  const [processed, setProcessed] = useState<ProcessingTake[]>([]), [processingWarnings, setProcessingWarnings] = useState<string[]>([]), [stage, setStage] = useState('original');
