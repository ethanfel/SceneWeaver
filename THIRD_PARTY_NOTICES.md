...
`h3_prompt_schema_core.mjs`, `h3_prompt_completion_core.mjs`, `h3_rich_prompt_editor_core.mjs` and `h3_reference_preview_core.mjs` in the same fixture directory are also unmodified GPL-3.0 copies from that nightly revision. They test real schema analysis, structure insertion, token completions and project reference records. `h3_chain_scene_prompt_editor.js` there is a small SceneWeaver-authored discovery fixture, not a copy of the native DOM editor. Production uses the installed helper modules in the ComfyUI tab.

`h3_prompt_history_core.mjs` is another unmodified GPL-3.0 fixture from H3 nightly `326453d71065f8031c160ea4d504678dab3e0cc1`. History adapter/browser tests use its real ancestry and revision eligibility helpers with synthetic revision storage. Native persistence, ownership and interrupted-write tests accompany draft H3 PR #61 (`1cd0e3cc6287d46e5fe52d20ea2524f6331703f8`). SceneWeaver imports the installed tree helper at runtime; this fixture and its mock HTTP backend are not a bundled H3 server.

`e2e/fixtures/h3-native/h3_project_asset_editor_core.mjs` is an unmodified GPL-3.0 test copy from [H3 nightly `326453d71065f8031c160ea4d504678dab3e0cc1`](https://github.com/ethanfel/ComfyUI-MiniMaxH3-Context-Loop/blob/326453d71065f8031c160ea4d504678dab3e0cc1/web/h3_project_asset_editor_core.mjs). It replaces the former audio-only stub and exercises the native aspect-ratio, megapixel and output-multiple rules. Production imports this helper from the installed H3 pack; the browser HTTP/storage fixture remains synthetic.
