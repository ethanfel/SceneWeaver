...
export function planTaskCapabilities(snapshot, planId) {
  ...
  task('asset-library', 'Organize folders and asset cards', assetsReady && caps.assetLibraryVersion === 1, assetsReady && caps.assetLibraryVersion === 1 ? 'Browse folders and inspect selected-Plan mentions, track groups and shared media. Conditional folder, membership, order, duplicate and delete controls are enabled only when the native library API advertises support; other Plans and saved-branch usage remain unverified.' : 'A current companion library bridge and the bound Carousel are required.');
