...
export function planTaskCapabilities(snapshot, planId) {
  ...
  task('plan-edit', 'Edit scene structure and chapter markers', writablePlan && caps.planAuthoringVersion === 1, !writablePlan ? planReason : caps.planAuthoringVersion === 1 ? 'Uses installed H3 helpers to prepare scene and chapter edits in the current draft. Applying uses the existing text source; checkpoints and saved editorial are separate.' : 'Refresh the ComfyUI tab with native H3 Plan authoring helpers installed. Prompt drafting remains available.');
  task('plan-settings', 'Edit duration, seeds and chapter resolution', writablePlan && caps.planSettingsVersion === 1, !writablePlan ? planReason : caps.planSettingsVersion === 1 ? 'Stages inherited/explicit duration and steps, generation and prompt seeds, shared direction and chapter boundaries/resolution. Requested seconds remain literal for the Python compiler; saved-cut timing is separate.' : 'Native H3 Plan settings exports are required. Scene structure support is checked independently.');
  ...
  task('asset-library', 'Organize folders and asset cards', assetsReady && caps.assetLibraryVersion === 1, assetsReady && caps.assetLibraryVersion === 1 ? 'Browse folders and inspect selected-Plan mentions, track groups and shared media. Other-project browsing and reviewed group copies use the current copy bridge. Conditional folder, membership, order, duplicate, copy and delete controls are enabled only when the native library API advertises support; other Plans and saved-branch usage remain unverified.' : 'A current companion library bridge and the bound Carousel are required.');
  task('asset-frame', 'Capture a saved clip frame', assetsReady && caps.assetFrameVersion === 1, assetsReady && caps.assetFrameVersion === 1 ? 'The main viewer captures the displayed saved revision at clip-local time. Reviewed saves require the updated H3 capture API and current project ownership; live previews and gaps are unavailable.' : 'A bound Asset Carousel and the current companion capture bridge are required.');
  task('asset-image', 'Crop and resize picture variants', assetsReady && caps.assetImageVersion === 1, assetsReady && caps.assetImageVersion === 1 ? 'Uses native image geometry, aspect ratio, megapixel sizing and output multiples. Reviewed saves require the updated H3 image-variant API and project ownership.' : 'A bound Asset Carousel and the installed H3 image-sizing helper are required.');
  ...
  task('prompt-tools', 'Prompt schema, structure and token tools', writablePlan && caps.promptToolsVersion === 1, caps.promptToolsVersion === 1 ? 'Uses installed H3 schema and completion helpers. Alias records are scoped to the connected Carousel; separately wired and scheduled references need dedicated binding adapters.' : 'The installed H3 prompt editor helpers are required. Plain scene drafting remains available.');
  task('prompt-history', 'Browse and restore saved prompt text', managed && document.status === 'resolved' && caps.promptHistoryVersion === 1, !managed ? managedReason : caps.promptHistoryVersion === 1 ? 'Reads the selected scene’s native revision tree and compares saved text with the editor. Loading text stays local until Stage and Apply. Save, fork, label, archive and delete are enabled only when the server advertises conditional history commands; named branches require an echoed scope.' : 'The installed native prompt-history tree and Plan helpers are required.');
